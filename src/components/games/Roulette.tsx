...
type BetType = 'straight' | 'split' | 'street' | 'corner' | 'line' | 'dozen' | 'column' | 'red' | 'black' | 'even' | 'odd' | 'low' | 'high';
...
export const Roulette = () => {
  ...
  const { placeBet, resolveBet } = useBetting(sessionId, 'roulette');
    ...
    for (const bet of placedBets) {
      ...
      switch (bet.type) {
        ...
        case 'black':
          ...
          break;
        case 'even':
          if (winningNumber % 2 === 0 && winningNumber !== 0) multiplier = 1;
          break;
        case 'odd':
      ...
      if (multiplier > 0) {
        payouts.push({
          ...
          amount: bet.amount * (multiplier + 1) // Include original bet
        });
      }
    }
      ...
      <div className="mb-8 w-full max-w-4xl">
        ...
        <div className="mb-4 grid grid-cols-12 gap-1">
          <button
            ...
            disabled={gamePhase !== 'betting'}
            className="col-span-1 h-12 rounded bg-green-600 font-bold text-white transition-transform hover:scale-105 disabled:opacity-50"
          >
            0
          </button>
