...
import { Coins, RotateCw, Plus, Minus, Trophy, Zap, Star } from 'lucide-react';
...
// Alien-western themed symbols
const SYMBOLS = {
  '👽': { weight: 1, payout: 50, name: 'Alien Sheriff' },
  '🛸': { weight: 2, payout: 25, name: 'UFO' },
  '🌌': { weight: 3, payout: 20, name: 'Galaxy' },
  '🚀': { weight: 4, payout: 15, name: 'Rocket' },
  '🌠': { weight: 5, payout: 10, name: 'Star' },
  '🌍': { weight: 6, payout: 5, name: 'Earth' }
} as const;

const ROWS = 3;
const COLS = 5;

export const AlienSaloon = () => {
  const { user } = useStore();
  ...
  const { 
    ...
    decrementBet,
    error: betError 
  } = useBetting(sessionId, 'alien-saloon', currencyType);

  // Initialize grid with empty arrays to prevent mutation
  ...
  const spin = async () => {
    ...
    try {
      // Animate spinning with cosmic effects
      let spins = 0;
  ...
  const calculateWinnings = (currentGrid: string[][]) => {
    ...
    const ufos = allSymbols.filter(s => s === '🛸').length;
    const stars = allSymbols.filter(s => s === '🌠').length;
    
    if (ufos >= 3) {
      newBonusSpins = ufos * 2; // 2 bonus spins per UFO
    }
    
    if (stars >= 3) {
      newMultiplier = Math.min(multiplier + stars - 2, 5); // Increase multiplier based on stars
    }
  ...
  return (
    <div className="flex min-h-screen flex-col items-center bg-gradient-to-b from-purple-900 via-indigo-900 to-blue-900 p-8">
      ...
      <div className="mt-8 rounded-lg bg-black/30 p-4 text-white backdrop-blur-sm">
        ...
        <div className="mt-4 space-y-2 text-sm text-gray-300">
          <p>🛸 3 or more UFOs award Bonus Spins</p>
          <p>🌠 3 or more Stars increase win multiplier</p>
        </div>
      </div>
    </div>
  );
