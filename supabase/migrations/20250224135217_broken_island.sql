...
BEGIN
  ...
  EXECUTE 'ALTER TABLE currency_balances ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE currency_transactions ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE currency_packages ENABLE ROW LEVEL SECURITY';
  EXECUTE 'ALTER TABLE currency_purchases ENABLE ROW LEVEL SECURITY';
  ...
  CREATE POLICY "Users can view their own balances" ON currency_balances FOR SELECT USING (auth.uid() = user_id);
  CREATE POLICY "Users can view their own transactions" ON currency_transactions FOR SELECT USING (auth.uid() = user_id);
  CREATE POLICY "Currency packages are viewable by everyone" ON currency_packages FOR SELECT USING (true);
  CREATE POLICY "Users can view their own purchases" ON currency_purchases FOR SELECT USING (auth.uid() = user_id);
END $$;
